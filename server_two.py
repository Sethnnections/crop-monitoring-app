...
def detect_fire_areas(ndvi, ndmi, lst):
    """Detect fire risk areas using vegetation indices and temperature"""
    # Create fire risk index
    fire_risk = (1 - ndvi) * (1 - ndmi) * (lst / 40)  # Normalize LST
    
    # Apply smoothing
    fire_risk = gaussian_filter(fire_risk, sigma=1)
    ...
    pixel_area = 0.0001  # hectares
    return {
        'high_risk_area': np.sum(high_risk) * pixel_area,
        'medium_risk_area': np.sum(medium_risk) * pixel_area,
        'low_risk_area': np.sum(low_risk) * pixel_area,
        'fire_count': np.sum(high_risk),
        'heatmap': fire_risk
    }
...
def detect_fires():
    try:
        ...
        ndvi_request = create_sentinelhub_request(
            ...
            bbox_to_dimensions(get_geojson_bbox(geojson), resolution=10),
            config
        )
        ...
        function evaluatePixel(samples) {
            return [samples.B10 * 0.1];  // Convert to °C
        }
        """
        lst_request = SentinelHubRequest(
            ...
            geometry=geometry,
            size=bbox_to_dimensions(get_geojson_bbox(geojson), resolution=60),
            config=config
        )
        _, lst_data = process_sh_data(lst_request.get_data())
